...
const { applyTick } = await import("../src/engine/simulation.js");
...
test("save puis load redonne un état équivalent (reprise après rechargement)", () => {
  ...
  applyTick(state, 5000);
  saveState(state);

  const { state: reloaded, fresh, rejected } = loadState(1700);
  assert.equal(fresh, false);
  assert.equal(rejected, false);
  assert.equal(reloaded.maps.water.totalLitersShipped, state.maps.water.totalLitersShipped);
});
...
test("une sauvegarde d'une version future inconnue est rejetée proprement", () => {
  localStorage.setItem(SAVE_KEY, JSON.stringify({ version: 999, money: 50 }));
